...
namespace Microsoft.TestTemplates.Acceptance.Tests
{
    ...
    public class AcceptanceTestBase
    {
        ...
        protected string _standardTestOutput = string.Empty;
        protected string _standardTestError = string.Empty;
        protected int _runnerExitCode = -1;
        ...
        {
            Execute(arguments, assertExecution);
        ...
        {
            Execute("test " + "--logger:Console;Verbosity=Detailed " + arguments);
            _standardTestError = Regex.Replace(_standardTestError, @"\s+", " ");
            _standardTestOutput = Regex.Replace(_standardTestOutput, @"\s+", " ");
        }
        ...
        public void InvokeDotnetNew(string templateName, string nameAs, string targetFramework = null, string language = null, string outputDirectory = null)
        {
            ...
            Execute($"new {templateName} -n {nameAs}{targetArgs}{languageArgs}{outputArgs}");
            _standardTestError = Regex.Replace(_standardTestError, @"\s+", " ");
            _standardTestOutput = Regex.Replace(_standardTestOutput, @"\s+", " ");
        }
        ...
        {
            _arguments = args;
            Execute(args, out _standardTestOutput, out _standardTestError, out _runnerExitCode);
            if (assertExecution)
            {
                Assert.AreEqual(0, _runnerExitCode, "'dotnet {0}' command failed, exit code: {1}, stdOut: {2}, stdErr: {3}", args, _runnerExitCode, _standardTestOutput, _standardTestError);
            }
        }
