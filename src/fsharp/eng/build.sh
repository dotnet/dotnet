...
function BuildSolution {
  ...
  if [ ! -f "$bootstrap_dir/fslex/fslex.dll" ]; then
    ...
    local args=" publish $repo_root/proto.proj $blrestore $bltools /p:Configuration=Proto /p:DotNetBuild=$product_build /p:DotNetBuildSourceOnly=$source_build $properties"
    echo $args
