...
namespace Microsoft.DotNet.SignTool
{
    ...
    internal sealed class ZipData
    {
...
#else
                // Workaround for https://github.com/dotnet/arcade/issues/16484 until https://github.com/dotnet/runtime/issues/74404 is fixed.
                // On Windows, use tar.exe to extract so hardlinks are resolved to regular files.
                // Windows tarballs use hardlinks for deduplication, which System.Formats.Tar doesn't fully support yet.
        ...
        {
            // Workaround for https://github.com/dotnet/arcade/issues/16484 until https://github.com/dotnet/runtime/issues/74404 is fixed.
            // System.Formats.Tar doesn't yet support hardlinks, and Windows tarballs use hardlinks for deduplication.
            // This is a temporary workaround until the runtime's tar library supports hardlinks.
            if (RuntimeInformation.IsOSPlatform(OSPlatform.Windows))
            {
        ...
        /// <summary>
        /// Read tar.gz entries by extracting with external tar.exe.
        /// This resolves hardlinks to regular files, which is needed on Windows where tarballs use hardlinks for deduplication.
        /// </summary>
        private static IEnumerable<ZipDataEntry> ReadTarGZipEntriesWithExternalTar(string archivePath, string tempDir, bool ignoreContent)
        {
            ...
            try
            {
                ...
                foreach (string path in Directory.EnumerateFiles(extractDir, "*", SearchOption.AllDirectories))
                {
                    string relativePath = path.Substring(extractDir.Length + 1).Replace(Path.DirectorySeparatorChar, '/');
                    using Stream stream = ignoreContent ? null : File.OpenRead(path);
                    yield return new ZipDataEntry(relativePath, stream);
                }
            }
            finally
