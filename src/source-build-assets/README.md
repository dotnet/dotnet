...
1. After the PR is merged to update a component, coordination is often needed in the darc dependency flows.
   The The source-build-assets source may need to flow in at the same time as the cooresponding changes in product repos which take a dependency on the new component version.
...
The [generate script](https://github.com/dotnet/The source-build-assets/blob/main/generate.sh)
...
> **Suggestion:** Open an [issue](https://github.com/dotnet/The source-build-assets/issues/new) that describes the packages to add and assign it to Copilot to do the work (example [issue](https://github.com/dotnet/The source-build-assets/issues/1324) and [resulting PR](https://github.com/dotnet/The source-build-assets/pull/1325)).
...
This repo should contain [issues](https://github.com/dotnet/The source-build-assets/issues) that are tied to reference, text-only, external, and target packages used by source build.
