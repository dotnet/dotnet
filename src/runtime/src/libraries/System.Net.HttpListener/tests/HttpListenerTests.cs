    ...
    public class HttpListenerTests
    {
        ...
        public class HttpListenerWindowsTests
        {
            ...
            public void EnableKernelResponeBuffering_DefaultIsDisabled()
            {
                ...
                {
                    listener.Start();
                    Assert.False(GetEnableKernelResponseBufferingValue(listener));
                }
            }
            ...
            public void EnableKernelResponeBuffering_Enabled()
            {
                ...
                {
                    listener.Start();
                    Assert.True(GetEnableKernelResponseBufferingValue(listener));
                }
            ...
            [ConditionalFact(nameof(Helpers) + "." + nameof(Helpers.IsWindowsImplementation))]
            public void EnableKernelResponeBuffering_ImmutableAfterStart()
            {
                AppContext.SetSwitch("System.Net.HttpListener.EnableKernelResponseBuffering", false);
            ...
            private bool GetEnableKernelResponseBufferingValue(HttpListener listener)
            {
                // We need enableKernelResponseBuffering which is internal so we get it using reflection.
                var prop = typeof(HttpListener).GetProperty("EnableKernelResponseBuffering", BindingFlags.Instance | BindingFlags.NonPublic);
                Assert.NotNull(prop);

                object? value = prop!.GetValue(listener);
