...
| PortableTargetRid | Valid RIDs | Portable rid for the target. | PortableTargetRid is a known rid from the graph that is compatible with the target. It is the same as `TargetRid` for portable build, or usable as a parent rid for non-portable builds. |
