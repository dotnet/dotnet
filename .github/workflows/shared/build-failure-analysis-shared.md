---
# Shared body for the build-failure-analysis workflow.
#
# Imported by build-failure-analysis.md. Keeps the prompt that delegates
# to the build-failure-analyst agent in one place.

description: "Shared body for build-failure-analysis workflow"
---
...
2. If `GH_AW_BUILD_OUTCOME == 'skipped'`,
   no binlogs were found — there is nothing to analyse. Call `noop`
   with the message `"No binlogs found — nothing to analyse."` and stop.
